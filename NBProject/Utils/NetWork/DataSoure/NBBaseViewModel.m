...
@property (nonatomic,strong,readwrite)NBHttpConfig *currentConfig;
...
//实例化一个基础的请求配置
- (NBHttpConfig *)mh_defaultHttpConfigWithApi:(NSString *)apiStr{
    NBHttpConfig *config = [NBHttpConfig defaultHttpConfig];
    config.apiStr = apiStr;
    return config;
}

//开启一个请求,对于开启一个接口，关注的应该是这个请求的配置，中间请求过程和结果处理无需关心，因为配置已经决定了请求和结果处理
- (void)startHttpWithRequestConfig:(NBHttpConfig *)model{
...
- (void)handleSuccessData:(id)data subscriber:(id<RACSubscriber>)subscriber{
    ...
    if([self checkDataWithClass:NSClassFromString(self.currentConfig.checkClassName) data:data subscriber:subscriber]){
        ...
        if(self.currentConfig.parserBlock){
            id adata = [data ln_parseMake:^(NBParserMarker *make) {
    ...
    if(!_baseSignal){
        ...
        _baseSignal = [RACSignal createSignal:^RACDisposable * _Nullable(id<RACSubscriber>  _Nonnull subscriber) {
            ...
            if(self.currentConfig.method == LNRequestMethodGet){
                ...
                [[NBNetworking shareNetworking] get:self.currentConfig.apiStr params:self.currentConfig.parameter progress:[self.currentConfig.progressBlock copy] success:^(id data) {
                    if(self.currentConfig.needShow){
                ...
                [[NBNetworking shareNetworking] post:self.currentConfig.apiStr params:self.currentConfig.parameter progress:[self.currentConfig.progressBlock copy] success:^(id data) {
                    if(self.currentConfig.needShow){
                        [MHHUDManager dismiss];
                    }
                    [self handleSuccessData:data subscriber:subscriber];
