...
- (void)viewDidLoad {
    [super viewDidLoad];
    _scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, MSafeAreaNaviBarHeight, MScreenW, MScreenH - MSafeAreaNaviBarHeight)];
    _scrollView.delegate = self;
    _scrollView.pagingEnabled = YES;
    _scrollView.contentSize = CGSizeMake(MScreenW * 4, MScreenH - MSafeAreaNaviBarHeight);
    _scrollView.bounces = NO;
    _scrollView.showsHorizontalScrollIndicator = NO;
    [self.view addSubview:_scrollView];
    for (int i = 0; i < 4 ; i++) {
        WSLAnimationView * view = [[WSLAnimationView alloc] initWithFrame:CGRectMake(i * MScreenW, 0, MScreenW, MScreenH - MSafeAreaNaviBarHeight)];
