...
#import "NBLoMasonryController.h"
...
@interface NBLoMasonryController ()
...
@implementation NBLoMasonryController
/*
...
- (void)mas_makeConstraintsWithBtns {
    [self.blackV mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.view).offset(MSafeAreaHeight_NaviBar + margir);
...
- (void)blackV_mas_update {
    ...
    [self.redV mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.edges.equalTo(self.view).insets(UIEdgeInsetsMake(MSafeAreaHeight_NaviBar + margir * 2, margir, MSafeAreaHeight_TabBar, margir));
    }];
}
