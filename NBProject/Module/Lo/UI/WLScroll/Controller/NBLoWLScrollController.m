...
#import "NBLoWLScrollController.h"
#import "WLScrollView.h"
#import "LableView.h"
#import "textView.h"
#import "SubView.h"
#import "NBMacros.h"
@interface NBLoWLScrollController ()<WLScrollViewDelegate>
{
    WLScrollView *wlScrView;
}
@end

@implementation NBLoWLScrollController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view, typically from a nib.
    UIView *bg = [[UIView alloc]initWithFrame:CGRectMake(0, 0, MScreenW, MScreenH)];
    ...
    for (int i = 0; i<4; i++) {
        ...
        btn.frame=CGRectMake(i*MScreenW/4, 100, MScreenW/4, 30);
...
- (WLSubView *)scrollView:(WLScrollView *)scrollView subViewFrame:(CGRect)frame cellAtIndex:(NSInteger)index{
    ...
    if (index == 4 || index == 1) {
        ...
        sub.im.image = [UIImage imageNamed:@"3.jpg"];
        return sub;
    }else if (index == 2){
        ...
        sub.img.image = [UIImage imageNamed:@"0.jpg"];
        return sub;
    }
    else  if (index == 0){
        ...
        sub.im.image = [UIImage imageNamed:@"1.jpg"];
        return sub;
    }else{
        LableView *sub = (LableView *)[scrollView dequeueReuseCellWithIdentifier:cellID];
        if (!sub) {
        ...
        sub.im.image = [UIImage imageNamed:@"2.jpg"];
        return sub;
    }
}
...
- (void)addWLScreenView1{
    ...
    wlScrView = nil;
    wlScrView = [[WLScrollView alloc]initWithFrame:CGRectMake(0, 150, MScreenW, 120)];
...
- (void)addWLScreenView4{
    [wlScrView removeFromSuperview];
    wlScrView = nil;
    wlScrView = [[WLScrollView alloc]initWithFrame:CGRectMake(0, 150, MScreenW, 400)];
    wlScrView.delegate = self;
